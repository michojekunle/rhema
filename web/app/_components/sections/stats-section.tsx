...
export function StatsSection({ stars }: { stars: number }) {
  ...
  return (
    ...
    >
      <Container>
        <div className="divide-border-strong grid grid-cols-1 sm:grid-cols-3 sm:divide-x">
          {items.map((item, i) => (
            <Reveal
              key={item.label}
              delay={i * 80}
            ...
            >
              <p className="text-5xl leading-none font-medium tracking-[-0.02em] text-foreground sm:text-6xl lg:text-[62px]">
                {item.value}
              </p>
